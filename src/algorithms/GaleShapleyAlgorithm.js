...
class minSpread extends BaseAlgorithm {
  calculateAllocation() {
    this.validateInput();
  ...
  function minSpread(graph, maxMatchingSize, numPref) {
    let spread = numPref;
    let ans = [-1, []];
    let finalRestrictedGraph;
    let matchA = [], matchH = [];

    for (let j = 0; j < numPref; j++) {
      
        let left = 1, right = numPref - j, result = right;
        while (left <= right) {
            const mid = Math.floor((left + right) / 2);
            const restrictedGraph = new Graph(graph.numStudents, graph.numCourses);
            
            for (let a = 1; a <= graph.numStudents; ++a) {
                for (let i = j; i < Math.min(mid + j, graph.adj[a].length); ++i) {
                    restrictedGraph.adj[a].push(graph.adj[a][i]);
                }
            }

            matchA = Array(graph.numStudents + 1).fill(0);
            matchH = Array(graph.numCourses + 1).fill(0);
            const [matchingSize, resultMatchA] = hopcroftKarp(restrictedGraph, matchA, matchH);

            if (matchingSize === maxMatchingSize) {
                result = mid;
                if (result < spread || ans[0] === -1) {
                    spread = result;
                    ans = [mid, resultMatchA];
                    finalRestrictedGraph = restrictedGraph;
                    matchH2 = matchH.slice();
                }
                right = mid - 1;
            } else {
                left = mid + 1;
    ...
    makeCoalitionFree(ans[1], matchH2, finalRestrictedGraph);
    ...
    // Main function
      ...
      const prefLength = Object.keys(this.preferences[1]).length;
      const result = minSpread(graph, maxMatchingSize[0], prefLength);
    
      console.log("Maximal Matching : " + maxMatchingSize[0]);
      console.log("Least Dissatisfaction Matching Size: " + result[0]);
     const matchA2=result[1];
